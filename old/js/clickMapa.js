...
function selectMunicipality(municipalityId, provinceColor, savedColors, normalizeText) {
    ...
    // Llamar a saveColors después de agregar el color
    saveColors(savedColors);
}

function saveColors(savedColors) {
    // Enviar los colores y municipios al backend
    fetch('http://localhost:3000/api/municipalities', {
        ...
        body: JSON.stringify(savedColors) // El cuerpo del POST contiene el objeto con los municipios y sus colores
    })
    .then(response => response.json())
    .then(data => {
        console.log('Municipios y colores guardados:', data);
    })
    .catch(error => {
        console.error('Error al guardar los municipios:', error);
    });
}
