...
export async function loadMap() {
    fetch(`mapa.svg?timestamp=${new Date().getTime()}`)
        ...
        .then(svg => {
            ...
            // 👇 Sincroniza savedColors con el backend
            async function syncWithBackend() {
                try {
                    const response = await fetch('/api/municipios/colors', {
                        ...
                        body: JSON.stringify(savedColors),
                    });

                    if (!response.ok) {
                        throw new Error('Errorea zerbitzarira sinkronizatzean.');
                    }

                    console.log('✅ Udalerriak eta koloreak gorde dira backend-ean');
                } catch (error) {
                    console.error('❌ Sinkronizazio hutsa:', error.message);
                }
            }

            applySavedColors();
            updateList();
            syncWithBackend(); // 🆕 Llamada aquí para guardar en backend
            //localStorage.removeItem('coloresMunicipios');
